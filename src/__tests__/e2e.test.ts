...
describe('End-to-End Tests', () => {
  ...
  beforeAll(() => {
    ...
    fs.copyFileSync(
      path.join(exampleDir, 'en.json'),
      path.join(tempDir, 'en.json')
    );
    
    if (fs.existsSync(path.join(exampleDir, 'glossary.json'))) {
      fs.copyFileSync(
        path.join(exampleDir, 'glossary.json'),
        path.join(tempDir, 'glossary.json')
      );
    }
  ...
  test('Full workflow: build-vector -> translate -> review -> search', () => {
    ...
    try {
      ...
      execSync(`node dist/index.js build-vector --source ${path.join(tempDir, 'en.json')} --context ${path.join(tempDir, 'src')}`, {
        stdio: 'pipe',
      });
      
      // Translate to Japanese
      execSync(`node dist/index.js translate --lang ja --source ${path.join(tempDir, 'en.json')} --dest ${path.join(tempDir, 'ja.json')}`, {
        stdio: 'pipe',
      });
      ...
      const searchResult = execSync(
        `node dist/index.js search "button" --lang ja --top-k 3`,
  ...
  test('Import command', () => {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(tempDir, 'ja.json'),
        JSON.stringify(sampleTranslation, null, 2)
      );
      ...
      const searchResult = execSync(
        `node dist/index.js search "cancel" --lang ja --top-k 1`,
        { stdio: 'pipe', encoding: 'utf-8' }
      );
